...
"use strict";
{
  ...
  Game_Interpreter.prototype.commandFesLocateMap203 = function (
    params = this._params
  ) {
  ...
  Game_Interpreter.prototype.commandFesScrollMap205 = function (
    params = this._params
  ) {
    if (!$gameParty.inBattle()) {
  ...
  Game_Map.prototype.isMovementSucceeded = function () {
    //未使用
    return true;
  };
  ...
  const _Game_Character_advanceMoveRouteIndex =
  ...
  Game_Map.prototype.locateToCharacter = function (eventId) {
    const character =
      eventId > 0 ? this.event(eventId) : this._interpreter.character(eventId);
  ...
  Game_Map.prototype.scrollToCharacterX = function (eventId) {
    ...
    if (character) {
      const dx = this.deltaX(
        character.x,
        this._displayX + $gamePlayer.centerX()
      );
      this.startScroll(dx < 0 ? 4 : 6, Math.abs(dx));
    }
  };
  ...
  Game_Map.prototype.scrollToCharacterY = function (eventId) {
    ...
    if (character) {
      const dy = this.deltaY(
        character.y,
        this._displayY + $gamePlayer.centerY()
      );
      this.startScroll(dy < 0 ? 8 : 2, Math.abs(dy));
    }
  };
