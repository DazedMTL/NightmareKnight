...
if (Utils.isOptionValid("test")) {
  ...
  (function () {
    ...
    var Params = {};
    Params.variableInputDigits = Math.min(
      Math.max(
        parseIntWithDefault(pluginParams["Variable input digits"], 9),
        1
      ),
      15
    );
    ...
    Params.mapGridColor =
      pluginParams["Map grid color"] || "rgba(255, 255, 255, 0.6)";
    Params.watchFontSize = Math.max(
      parseIntWithDefault(pluginParams["Watch window font size"], 18),
      1
    );
    Params.watchUpdateColor =
      pluginParams["Watch update color"] || "rgba(255, 224, 192, 0.6)";
    Params.watchWindowPosition = parseIntWithDefault(
      pluginParams["Watch window position"],
      0
    );
    Params.watchWindowWidth = Math.max(
      parseIntWithDefault(pluginParams["Watch window width"], 360),
      1
    );
    Params.WatchWindowPosition = Math.min(
      Math.max(
        parseIntWithDefault(
          pluginParams["Watch window position"],
    ...
    KMS.DebugUtil.registerCommand = function (command) {
      ...
      if (exists) {
        // 登録済み
        console.log(
          '[%s] Command "%s" is already registered',
          Const.pluginCode,
          command.id
        );
        return false;
    ...
    KMS.DebugUtil.registerDatabaseReloadHandler = function (handler) {
      ...
      if (exists) {
        // 登録済み
        console.log(
          "[%s] Specified reload handler is already registered",
          Const.pluginCode
        );
        return false;
      }
    ...
    DebugWatchItemForSave.prototype.toActualItem = function () {
      ...
      switch (this._type) {
        ...
        default:
          console.error(
            "%s: toDisplayItem() failed. Invalid item type (%d)",
            Const.pluginCode,
            this._type
          );
          return null;
    ...
    Object.defineProperties(TextManager.DebugUtil, {
      ...
      // コマンド名
      itemNumber: simpleGetter(
        isJp ? "アイテム所持数変更" : "Edit item numbers"
      ),
      ...
      loadDatabase: simpleGetter(
        isJp ? "データベース再読み込み" : "Reload database"
      ),
      transfer: simpleGetter(isJp ? "場所移動" : "Transfer"),
      switchVar: simpleGetter(
        isJp ? "スイッチ・変数操作" : "Edit switches/variables"
      ),
      selfSwitch: simpleGetter(
        isJp ? "セルフスイッチ操作" : "Edit self-switches"
      ),
      systemValue: simpleGetter(isJp ? "システム値操作" : "Edit system values"),
      watchWindow: simpleGetter(
        isJp ? "スイッチ・変数監視" : "Watch switches/variables"
      ),
      ...
      reserveParty: simpleGetter(
        isJp ? "パーティ外メンバー" : "Reserve member"
      ),
      ...
      encounterRandom: simpleGetter(
        isJp ? "現在のマップからランダム" : "Random in current map"
      ),
    ...
    Sprite_DebugMapDestination.prototype = Object.create(
      Sprite_Destination.prototype
    );
    Sprite_DebugMapDestination.prototype.constructor =
      Sprite_DebugMapDestination;
     ...
     * 可視領域を設定
     */
    Sprite_DebugMapDestination.prototype.setVisibleArea = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugNumberInput.prototype = Object.create(
    ...
    Object.defineProperty(Window_DebugNumberInput.prototype, "number", {
      get: function () {
        return this._number * (this._isNegative ? -1 : 1);
      },
      configurable: true,
    });
    ...
    Window_DebugNumberInput.prototype.windowWidth = function () {
      ...
      if (isItemObject(this._caption)) {
        captionWidth +=
          this.textWidth(this._caption.name) + Window_Base._iconWidth + 4;
    ...
    Window_DebugNumberInput.prototype.maxCols = function () {
      return (
        Window_NumberInput.prototype.maxCols.call(this) +
     ...
     * 値の入力を開始
     */
    Window_DebugNumberInput.prototype.start = function (
      number,
      maxDigits,
      callback
    ) {
    ...
    Window_DebugItemList.prototype.drawItemNumber = function (
      item,
      x,
      y,
      width
    ...
    Window_DebugActorList.prototype = Object.create(
      Window_Selectable.prototype
    );
    Window_DebugActorList.prototype.constructor = Window_DebugActorList;

    Window_DebugActorList.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugActorParam.prototype = Object.create(
      Window_Selectable.prototype
    );
    Window_DebugActorParam.prototype.constructor = Window_DebugActorParam;

    Window_DebugActorParam.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugActorSkill.prototype = Object.create(
      Window_SkillList.prototype
    );
    Window_DebugActorSkill.prototype.constructor = Window_DebugActorSkill;

    Window_DebugActorSkill.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugPartyListBase.prototype = Object.create(
      Window_Selectable.prototype
    );
    Window_DebugPartyListBase.prototype.constructor = Window_DebugPartyListBase;

    Window_DebugPartyListBase.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugPartyListBase.prototype.drawItem = function (index) {
      ...
      this.drawText(
        "%1%2".format(TextManager.levelA, level),
        rect.x,
        rect.y,
        rect.width,
        "right"
    ...
    Window_DebugActivePartyList.prototype = Object.create(
      Window_DebugPartyListBase.prototype
    );
    Window_DebugActivePartyList.prototype.constructor =
      Window_DebugActivePartyList;

    Window_DebugActivePartyList.prototype.initialize = function (
      x,
      y,
      width,
      height
      ...
      Window_DebugPartyListBase.prototype.initialize.call(
        this,
        x,
        y,
        width,
     ...
     * 並び替え状態を設定
     */
    Window_DebugActivePartyList.prototype.setFormationChangeMode = function (
      mode
    ) {
    ...
    Window_DebugActivePartyList.prototype.drawItemBackground = function (
      index
    ) {
    ...
    Window_DebugReservePartyList.prototype = Object.create(
      Window_DebugPartyListBase.prototype
    );
    Window_DebugReservePartyList.prototype.constructor =
      Window_DebugReservePartyList;

    Window_DebugReservePartyList.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugMapTilePosition.prototype = Object.create(
      Window_Base.prototype
    );
    Window_DebugMapTilePosition.prototype.constructor =
      Window_DebugMapTilePosition;
     ...
     * マップサイズの設定
     */
    Window_DebugMapTilePosition.prototype.setMapSize = function (
      width,
      height
    ) {
    ...
    Window_DebugMapTilePosition.prototype.refresh = function () {
      ...
      this.drawText(
        TextManager.DebugUtil.tilePosition,
        0,
        this.lineHeight(),
        160
      );
    ...
    Window_DebugMapTile.prototype.makeItemList = function () {
      ...
      this._data = Array.apply(null, { length: tileCount }).map(
        Number.call,
        Number
      );
    };
    ...
    Window_DebugMapTile.prototype.refreshGridSprite = function () {
      ...
      var dispHeight = alignSize(
        Math.min(dispSize.height, mapHeight),
        tileHeight
      );
    ...
    Window_DebugEventList.prototype = Object.create(
      Window_Selectable.prototype
    );
    Window_DebugEventList.prototype.constructor = Window_DebugEventList;

    Window_DebugEventList.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugSelfSwitchList.prototype = Object.create(
      Window_Selectable.prototype
    );
    Window_DebugSelfSwitchList.prototype.constructor =
      Window_DebugSelfSwitchList;

    Window_DebugSelfSwitchList.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugSystemValue.prototype = Object.create(
      Window_Selectable.prototype
    );
    ...
    Window_DebugSystemValue.DataTypeConfig = {
      ...
      battleCount: {
        type: "uint",
        get: function () {
          return $gameSystem._battleCount;
        },
        set: function (value) {
          $gameSystem._battleCount = value;
        },
      },
      winCount: {
        type: "uint",
        get: function () {
          return $gameSystem._winCount;
        },
        set: function (value) {
          $gameSystem._winCount = value;
        },
      },
      escapeCount: {
        type: "uint",
        get: function () {
          return $gameSystem._escapeCount;
        },
        set: function (value) {
          $gameSystem._escapeCount = value;
        },
      },
      saveCount: {
        type: "uint",
        get: function () {
          return $gameSystem._saveCount;
        },
        set: function (value) {
          $gameSystem._saveCount = value;
        },
      },
    };
    ...
    Window_DebugWatch.prototype.createUpdateSprites = function () {
      ...
      var count = Math.max(
        Math.floor(
          (Graphics.boxHeight - this.y - this.standardPadding() * 2) /
            lineHeight
        ),
        1
      );
    ...
    Window_DebugWatchItemList.prototype = Object.create(
      Window_Selectable.prototype
    );
    ...
    Window_DebugWatchItemList.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugWatchItemEdit.prototype = Object.create(
      Window_Selectable.prototype
    );
    Window_DebugWatchItemEdit.prototype.constructor = Window_DebugWatchItemEdit;

    Window_DebugWatchItemEdit.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Window_DebugWatchItemEdit.prototype.drawItem = function (index) {
      ...
      switch (item) {
        ...
        case "remove":
          this.changeTextColor(this.systemColor());
          this.drawText(
            TextManager.DebugUtil.watchItemRemove,
            rect.x + 40,
            rect.y,
            rect.width
          );
    ...
    Window_DebugTroopList.prototype = Object.create(
      Window_Selectable.prototype
    );
    Window_DebugTroopList.prototype.constructor = Window_DebugTroopList;

    Window_DebugTroopList.prototype.initialize = function (
      x,
      y,
      width,
      height
    ...
    Scene_Map.prototype.createDebugWatchWindow = function () {
      ...
      this._debugWatchWindow = new Window_DebugWatch(
        0,
        0,
        Params.watchWindowWidth,
        100
      );
    ...
    Scene_Debug.prototype.createEditWindow = function () {
      ...
      this._editWindow.updateSwitch = this._editWindow.updateVariable =
        function () {};
    };
    ...
    Scene_Debug.prototype.createCommandWindow = function () {
      ...
      this._commandWindow.setHandler(
        "actorParam",
        this.onSelectActor.bind(this)
      );
      this._commandWindow.setHandler(
        "member",
        this.onSelectPartyMember.bind(this)
      );
      this._commandWindow.setHandler("reloadMap", this.onReloadMap.bind(this));
      this._commandWindow.setHandler(
        "loadDatabase",
        this.onReloadDatabase.bind(this)
      );
      this._commandWindow.setHandler(
        "transfer",
        this.onTransferMapStart.bind(this)
      );
      this._commandWindow.setHandler(
        "switchVar",
        this.onSwitchVarStart.bind(this)
      );
      this._commandWindow.setHandler(
        "selfSwitch",
        this.onSelfSwitchMapStart.bind(this)
      );
      this._commandWindow.setHandler(
        "systemValue",
        this.onSelectSystemValue.bind(this)
      );
      this._commandWindow.setHandler(
        "watchWindow",
        this.onWatchWindow.bind(this)
      );
      this._commandWindow.setHandler(
        "encounter",
        this.onForceEncounter.bind(this)
      );
    ...
    Scene_Debug.prototype.createActorParamWindow = function () {
      ...
      this._actorParamWindow.setHandler(
        "cancel",
        this.onActorParamCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.createActorSkillWindow = function () {
      ...
      var wy = this._helpWindow.y + this._helpWindow.height;
      var wh = Graphics.boxHeight - wy;
      this._actorSkillWindow = new Window_DebugActorSkill(
        0,
        wy,
        Graphics.boxWidth,
        wh
      ...
      this._actorSkillWindow.setHandler(
        "cancel",
        this.onActorSkillCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.createPartyWindow = function () {
      ...
      this._activePartyWindow.setHandler(
        "formation",
        this.onActivePartyFormationStart.bind(this)
      );
      this._activePartyWindow.setHandler(
        "cancel",
        this.onActivePartyCancel.bind(this)
      );
      this.addWindow(this._activePartyWindow);

      this._reservePartyWindow = new Window_DebugReservePartyList(
        ww,
        wy,
        ww,
        wh
      );
      this._reservePartyWindow.setHandler(
        "ok",
        this.onReservePartyOk.bind(this)
      );
      this._reservePartyWindow.setHandler(
        "cancel",
        this.onReservePartyCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.createMapTileWindow = function () {
      ...
      this._mapTileWindow.setHandler(
        "cancel",
        this.onTransferMapTileCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.createEventListWindow = function () {
      ...
      this._eventListWindow.setHandler(
        "ok",
        this.onSelfSwitchEventListOk.bind(this)
      );
      this._eventListWindow.setHandler(
        "cancel",
        this.onSelfSwitchEventListCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.createSelfSwitchListWindow = function () {
      ...
      var wh = Graphics.boxHeight - wy;
      this._selfSwitchListWindow = new Window_DebugSelfSwitchList(
        wx,
        wy,
        ww,
        wh
      );
      this._selfSwitchListWindow.setHandler(
        "ok",
        this.onSelfSwitchListOk.bind(this)
      );
      this._selfSwitchListWindow.setHandler(
        "cancel",
        this.onSelfSwitchListCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.createSystemValueWindow = function () {
      ...
      this._systemValueWindow.x =
        (Graphics.boxWidth - this._systemValueWindow.width) / 2;
      this._systemValueWindow.setHandler("ok", this.onSystemValueOk.bind(this));
      this._systemValueWindow.setHandler(
        "cancel",
        this.onSystemValueCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.createWatchListWindow = function () {
      ...
      this._watchListWindow.setHandler(
        "cancel",
        this.onWatchListCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.createWatchEditWindow = function () {
      ...
      this._watchEditWindow.setHandler(
        "cancel",
        this.onWatchEditCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.isReady = function () {
      return (
        _Scene_Debug_isReady.call(this) &&
        DataManager.isDatabaseLoaded() &&
        DataManager.isMapLoaded() &&
        (!this._isLoadingTemporallyMap ||
          DataManager.isKmsTemporallyMapLoaded())
      );
    };
    ...
    Scene_Debug.prototype.onItemOk = function () {
      this._numberWindow.setHandler(
        "cancel",
        this.onItemNumberCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.onActorParamOk = function () {
      ...
      this._numberWindow.setHandler(
        "cancel",
        this.onActorParamInputCancel.bind(this)
      );
      this._numberWindow.start(
        number,
        digits,
        this.onActorParamInputOk.bind(this)
      );
    };
    ...
    Scene_Debug.prototype.onActivePartyFormationStart = function () {
      ...
      this._activePartyWindow.setHandler(
        "ok",
        this.onActivePartyFormationOk.bind(this)
      );
      this._activePartyWindow.setHandler(
        "formation",
        this.onActivePartyFormationOk.bind(this)
      ...
      this._activePartyWindow.setHandler(
        "cancel",
        this.onActivePartyFormationCancel.bind(this)
      );
    };
    ...
    Scene_Debug.prototype.onTransferMapStart = function () {
      ...
      this._mapListWindow.setHandler(
        "cancel",
        this.onTransferMapListCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.onSwitchVarOk = function () {
      ...
      switch (this._editWindow.getMode()) {
        ...
        case "variable":
          {
            // 変数の入力
            var caption = "%1:%2".format(
              id.padZero(4),
              this._editWindow.itemName(id)
            );
            ...
            this._numberWindow.setHandler(
              "cancel",
              this.onSwitchVarInputCancel.bind(this)
            );
            this._numberWindow.start(
              number,
              digits,
              this.onSwitchVarInputOk.bind(this)
            );
          }
          break;
    ...
    Scene_Debug.prototype.onSelfSwitchMapStart = function () {
      ...
      this._mapListWindow.setHandler(
        "ok",
        this.onSelfSwitchMapListOk.bind(this)
      );
      this._mapListWindow.setHandler(
        "cancel",
        this.onSelfSwitchMapListCancel.bind(this)
      );
    ...
    Scene_Debug.prototype.onSystemValueOk = function () {
      ...
      this._numberWindow.setHandler(
        "cancel",
        this.onSystemValueInputCancel.bind(this)
      );
      this._numberWindow.start(
        number,
        digits,
        this.onSystemValueInputOk.bind(this)
      );
    };
    ...
    Scene_Debug.prototype.onWatchSelectSwitchVarStart = function () {
      // ハンドラの切り替え
      this._rangeWindow.setHandler(
        "cancel",
        this.onWatchSelectSwitchVarCancel.bind(this)
      );
      this._editWindow.setHandler(
        "ok",
        this.onWatchSelectSwitchVarOk.bind(this)
      );
