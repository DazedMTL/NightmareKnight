...
*/
const AlchemySystemPluginName = document.currentScript
...
(function (AlchemySystem) {
  ...
  const PP = PluginParamsParser.parse(
    PluginManager.parameters(AlchemySystemPluginName),
    typeDefine
  );
  ...
  const DisplayItemCategory =
    PP.DisplayItemCategory == null ? true : PP.DisplayItemCategory;
  const DisplayWeaponCategory =
    PP.DisplayWeaponCategory == null ? true : PP.DisplayWeaponCategory;
  const DisplayArmorCategory =
    PP.DisplayArmorCategory == null ? true : PP.DisplayArmorCategory;
  ...
  class PartyItemUtils {
    static partyItemCount(itemInfo) {
      ...
      if (EnableIncludeEquipItem) {
        if (itemInfo.type === "weapon") {
          count += this._allPartyEquipWeapons().filter(
            (item) => item && item.id === itemInfo.id
          ).length;
        } else if (itemInfo.type === "armor") {
          count += this._allPartyEquipArmors().filter(
            (item) => item && item.id === itemInfo.id
          ).length;
        }
      }
    ...
    static _allPartyEquipWeapons() {
      const equipSlotItems = $gameParty
        .members()
        .flatMap((actor) => actor._equips);
      const weaponItems = equipSlotItems.filter(
        (item) => item.isWeapon() && item.itemId() > 0
      );
    ...
    static _allPartyEquipArmors() {
      const equipSlotItems = $gameParty
        .members()
        .flatMap((actor) => actor._equips);
      const armorItems = equipSlotItems.filter(
        (item) => item.isArmor() && item.itemId() > 0
      );
      ...
      if (gainCount > 0) {
        ...
        if (EnableIncludeEquipItem && itemInfo.type !== "item") {
          ...
          if (purgeCount > partyItemCount) {
            throw new Error(
              `purgeCount(${purgeCount}) is over all has item count(${partyItemCount})`
            );
          }
          const partyItemCountWithoutEquips =
            this._partyItemCountWithoutEquips(itemInfo);
          if (partyItemCountWithoutEquips >= purgeCount) {
            ...
            this._purgePartyEquipItem(
              itemInfo,
              purgeCount - partyItemCountWithoutEquips
            );
          }
        } else {
          $gameParty.gainItem(itemData, gainCount);
    ...
    static _purgePartyEquipItem(itemInfo, purgeCount) {
      ...
      for (const actor of $gameParty.members()) {
        for (let i = 0; i < actor._equips.length; i++) {
          ...
          if (itemInfo.type === "weapon") {
            if (
              actor._equips[i].isWeapon() &&
              itemInfo.id === actor._equips[i].itemId()
            )
              purgeFlag = true;
          } else if (itemInfo.type === "armor") {
            if (
              actor._equips[i].isArmor() &&
              itemInfo.id === actor._equips[i].itemId()
            )
              purgeFlag = true;
          }
  ...
  class AlchemyRecipe {
    ...
    static fromRecipeData(recipeData) {
      ...
      const targetItemInfo = new ItemInfo(
        recipeData.target[0],
        recipeData.target[1]
      );
    ...
    maxMakeItemCount() {
      ...
      const maxRemainingCount =
        $gameParty.maxItems(targetItem) -
        PartyItemUtils.partyItemCount(this._targetItemInfo);
      ...
      return makeItemCount > maxRemainingCount
        ? maxRemainingCount
        : makeItemCount;
    }
    ...
    makeItem(targetItemCount) {
      for (const tag in this._materials) {
        ...
        PartyItemUtils.gainPartyItem(
          material.itemInfo,
          -material.count * targetItemCount
        );
      }
  ...
  class Scene_Alchemy extends Scene_MenuBase {
    ...
    createSelectRecipesWindow() {
      this._windowSelectRecipes = new Window_SelectRecipes(
        this.selectRecipesWindowRect()
      );
      this._windowSelectRecipes.setHandler(
        "ok",
        this.selectRecipesOk.bind(this)
      );
      this._windowSelectRecipes.setHandler(
        "select",
        this.selectRecipesSelect.bind(this)
      );
      this._windowSelectRecipes.setHandler(
        "cancel",
        this.selectRecipesCancel.bind(this)
      );
    ...
    createNumberWindow() {
      ...
      this._numberWindow.setHandler(
        "changeNumber",
        this.onNumberChange.bind(this)
      );
      this._numberWindow.setHandler("cancel", this.onNumberCancel.bind(this));
      this.addWindow(this._numberWindow);
    }
    createRecipeDetailWindow() {
      this._windowRecipeDetail = new Window_RecipeDetail(
        this.recipeDetailWindowRect()
      );
    ...
    categoryWindowRect() {
      ...
      const ww = EnabledGoldWindow
        ? Graphics.boxWidth - goldWindowRect.width
        : Graphics.boxWidth;
      const wh =
        EnabledCategoryWindow || EnabledGoldWindow
    ...
    selectRecipesOk() {
      ...
      this._numberWindow.setup(
        recipe.targetItemData(),
        recipe.maxMakeItemCount(),
        2
      );
  ...
  class Window_RecipeDetail extends Window_Base {
    ...
    drawPossession() {
      ...
      this.resetTextColor();
      this.drawText(
        PartyItemUtils.partyItemCount(this._recipe.targetItemInfo()),
        x,
        y,
        width,
        "right"
    ...
    drawMaterials() {
      ...
      for (const tag in recipe.materials()) {
        ...
        const needItemCount =
          recipe.needItemCount(material.itemInfo) * this._numberWindow.number();
    ...
    drawTotalPrice() {
      ...
      this.drawText(
        ...
        x,
        y,
        width - this.textWidth(currentUnit),
        "right"
      );
      this.changeTextColor(this.systemColor());
