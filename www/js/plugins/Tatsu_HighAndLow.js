...
(function () {
  ...
  HighAndLowP.betCoinList = getArgJson(
  ...
  HighAndLowP.betCoinSelectPosX = Number(
    parameters["betCoinSelectPosX"] || 100
  );
  HighAndLowP.betCoinSelectPosY = Number(
    parameters["betCoinSelectPosY"] || 294
  );
  HighAndLowP.betCoinSelectHeight = Number(
    parameters["betCoinSelectHeight"] || 150
  );
  HighAndLowP.betCoinSelectWidth = Number(
    parameters["betCoinSelectWidth"] || 150
  );

  HighAndLowP.autoAdjustPositionAndSizeStr = String(
    parameters["autoAdjustPositionAndSize"] || "true"
  );
  ...
  HighAndLowP.coinWindowStringSize = Number(
    parameters["coinWindowStringSize"] || 19
  );
  ...
  Scene_HighAndLowScene.prototype.initialize = function () {
    ...
    var isAutoAdjustCoinWindow =
      String(HighAndLowP.autoAdjustPositionAndSizeStr) === "true"
        ? true
        : false;
  ...
  Scene_HighAndLowScene.prototype.createBackground = function () {
    ...
    if (
      HighAndLowP.backgroundImage &&
      HighAndLowP.backgroundImage != null &&
      HighAndLowP.backgroundImage != "" &&
      HighAndLowP.backgroundImage != "null"
    ) {
      this._backgroundSprite.bitmap = ImageManager.loadBitmap(
        "img/highLow/",
        HighAndLowP.backgroundImage.substr(imgHighLowDirStr.length)
      );
    } else {
  ...
  Scene_HighAndLowScene.prototype.create = function () {
    ...
    if (HighAndLowP.coinUse) {
      ...
      this._resultBetCoinWindow.setMyDispCoin(
        $gameVariables.value(HighAndLowP.coinVariable)
      );
    } else {
      this._step = "setup";
    }
  ...
  Scene_HighAndLowScene.prototype.start = function () {
    ...
    if (HighAndLowP.coinUse) {
      if (isNaN(HighAndLowP.coinVariable)) {
        alert(
          "パラメータ：「coinVariable」の設定が正しくありません。変数番号を設定してください。"
        );
        this.popScene();
      } else {
      ...
      if (!HighAndLowP.betCoinList) {
        alert(
          "パラメータ：「betCoinList」の設定が正しくありません。正しく枚数のリストを設定してください。"
        );
        this.popScene();
      } else {
    ...
    if (HighAndLowP.useBgm) {
      ...
      if (bgm != "" && bgm != null) {
        AudioManager.playBgm({
          name: bgm.substr(tmpFolder.length),
          volume: 80,
          pitch: 100,
          pan: 0,
        });
  ...
  Scene_HighAndLowScene.prototype.betOK = function () {
    ...
    if (nowCoin < coin) {
      if (this._InsufficientCoinsMsgWindow == undefined) {
        ...
        this._InsufficientCoinsMsgWindow.drawText(
          "Not Enough Coins",
          0,
          0,
          Graphics.width,
  ...
  Scene_HighAndLowScene.prototype.HandOutCard = function () {
    ...
    for (i = 0; i < 10; i++) {
      ...
      tmpSprite.bitmap = ImageManager.loadBitmap(
        "img/tPoker/",
        tmpCard.getMark() + tmpNum,
        0,
        false
      );
    ...
    if (HighAndLowP.coinUse) {
      if (HighAndLowP.isAutoAdjustCoinWindow) {
        this._resultBetCoinWindow.x =
          this._handWindow.x + this._handWindow.width;
      }
    }
  ...
  Scene_HighAndLowScene.prototype.resultAfter = function () {
    if (this._myHandWindow.getCardIndos().length > 0) {
      ...
      mySprite.x =
        mySprite.x + (140 + this._myHandWindow.getUsedCards().length * 70);
      mySprite.opacity = 255;
      mySprite.scale.x = 0.3;
      mySprite.scale.y = 0.3;
      enemySprite.x =
        enemySprite.x + (140 + this._handWindow.getUsedCards().length * 70);
  ...
  Scene_HighAndLowScene.prototype.coinPlus = function () {
    if (HighAndLowP.coinUse) {
      ...
      $gameVariables.setValue(
        HighAndLowP.coinVariable,
        Number(this._getCoin) + Number(tmpCoin)
      );
    }
  };
  ...
  Scene_HighAndLowScene.prototype.nextBattle = function () {
    ...
    if (this._confirmLoseMsgWindow == undefined) {
      ...
      this._confirmLoseMsgWindow.drawText(
    ...
    if (this._battleResult) {
      ...
      if (this._myHandWindow.getCardIndos().length > 0) {
        this._confirmWinMsgWindow.drawText(
    ...
    if (this._confirmMsgSelectWindow == undefined) {
      ...
      this._confirmMsgSelectWindow.setHandler(
        "ok",
        this.confirmNext.bind(this)
      );
  ...
  Scene_HighAndLowScene.prototype.viewGetCoinMsg = function () {
    ...
    if (this._WindowLastExitMsg.isClosed()) {
      ...
      this._WindowLastExitMsg.drawText(
        "The match is over",
        0,
        0,
        Graphics.width,
  ...
  Window_HighAndLow_Select.prototype = Object.create(
  ...
  Window_continue_Select.prototype.initialize = function () {
    ...
    Window_Selectable.prototype.initialize.call(
      this,
      HighAndLowP.betCoinSelectPosX,
      HighAndLowP.betCoinSelectPosY,
      150,
      windowHeight
    );
    this._data = new Array();
  ...
  Window_BetCoin_Window.prototype.initialize = function () {
    if (HighAndLowP.isAutoAdjustCoinWindow) {
      ...
      Window_Base.prototype.initialize.call(
        this,
        HighAndLowP.coinWindowX,
        HighAndLowP.coinWindowY,
        HighAndLowP.coinWindowWidth,
        HighAndLowP.coinWindowHeight
      );
    }
