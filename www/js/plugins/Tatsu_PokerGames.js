...
(function () {
  ...
  var parameterNotNull = function () {
    ...
    if (isRpgMakerMV()) {
      ...
      var para = PluginManager.parameters(
        decodeURIComponent(document.currentScript.src).match(
          /^.*\/js\/plugins\/(.+)\.js$/
        )[1]
      );
  ...
  TatsuPokerGameP.betInfoStr = String(
    parameters["betInfoStr"] || TATSU_POKER_MSG_KAKEKINWO
  );
  TatsuPokerGameP.winMessage1 = String(
    parameters["winMessage1"] || TATSU_POKER_MSG_WINMESSA1
  );
  TatsuPokerGameP.winMessage2 = String(
    parameters["winMessage2"] || TATSU_POKER_MSG_WINMESSA2
  );

  TatsuPokerGameP.loseMessage1 = String(
    parameters["loseMessage1"] || TATSU_POKER_MSG_LOSE_MESSAGE1
  );
  TatsuPokerGameP.loseMessage2 = String(
    parameters["loseMessage2"] || TATSU_POKER_MSG_LOSE_MESSAGE2
  );

  TatsuPokerGameP.notCoinMessage = String(
    parameters["notCoinMessage"] || TATSU_POKER_MSG_NOTCOIN
  );
  TatsuPokerGameP.backgroundImage = String(
    parameters["backgroundImage"] || null
  );
  TatsuPokerGameP.startButonFrameXPos = Number(
    parameters["startButonFrameXPos"] || 288
  );
  TatsuPokerGameP.startButonFrameYPos = Number(
    parameters["startButonFrameYPos"] || 539
  );
  TatsuPokerGameP.betCoinList = getArgJson(
    parameters["betCoinList"] || [1, 10, 100]
  );
  TatsuPokerGameP.betCoinListWindowWidth = Number(
    parameters["betCoinListWindowWidth"] || 100
  );
  TatsuPokerGameP.betCoinListWindowHeight = Number(
    parameters["betCoinListWindowHeight"] || 150
  );
  ...
  TatsuPokerGameP.nowCoinWindowWidth = Number(
    parameters["nowCoinWindowWidth"] || 235
  );
  TatsuPokerGameP.nowCoinPosHeight = Number(
    parameters["nowCoinPosHeight"] || 70
  );
  ...
  TatsuPokerGameP.betCoinWindowWidth = Number(
    parameters["betCoinWindowWidth"] || 235
  );
  TatsuPokerGameP.betCoinPosHeight = Number(
    parameters["betCoinPosHeight"] || 70
  );
  ...
  TatsuPokerGameP.yesNoWindowWidth = Number(
    parameters["yesNoWindowWidth"] || 120
  );
  TatsuPokerGameP.yesNoWindowHeight = Number(
    parameters["yesNoWindowHeight"] || 120
  );

  TatsuPokerGameP.rsfPercentageSwitch = Number(
    parameters["rsfPercentageSwitch"] || 0
  );
  ...
  TatsuPokerGameP.royalstraightFlushSe = String(
    parameters["royalstraightFlushSe"] || null
  );
  ...
  CardInfo.prototype.initialize = function (
    sprite,
    num,
    mark,
    isOpen,
  ...
  Scene_PokerScene.prototype.initialize = function () {
    ...
    TatsuPokerGameP.useBGM =
      String(TatsuPokerGameP.useBGMStr) === "true" ? true : false;
  ...
  Scene_PokerScene.prototype.initializeRoleInfo = function () {
    ...
    if (
      TatsuPokerGameP.roleStruct != undefined &&
      TatsuPokerGameP.roleStruct != null &&
      TatsuPokerGameP.roleStruct.length > 0
    ) {
      let rtnAr = {};
  ...
  Scene_PokerScene.prototype.create = function () {
    ...
    this._WindowMsg = new Window_MessageInfo_Window(
      messageCneterXPos,
      230,
      messageWindowWidth,
      145
    ...
    this._nowCoinWindows.reDrawCoin(
      $gameVariables.value(TatsuPokerGameP.coinVariable)
    );
    ...
    let roleAndMagnificationHalfLength = Math.floor(
      roleAndMagnificationLength / 2
    );
    let xline = 0;
    let yline = 0;
    ...
    for (let i = 0; i < roleAndMagnificationLength; i++) {
      ...
      for (let j = 0; j < roleAndScore.length; j++) {
        ...
        this._scoreInfoWindow.drawText(
          str,
          j * 120 + xline * 400,
          yline * 40,
          250,
  ...
  Scene_PokerScene.prototype.selectBet = function () {
    ...
    if (this._turn == TURN_SELECTBET && index >= 0) {
      ...
      let zanCoin =
        $gameVariables.value(TatsuPokerGameP.coinVariable) - this._betCoin;
  ...
  Scene_PokerScene.prototype.createImgBord = function () {
    ...
    this._royalStraightFlushAnimationSprite = new Sprite(
      loadBitmapByTPokerPluginFolder("royalstraighflushAnimation")
    );
    this.addChild(this._mainBaseBord);
    this.addChild(this._explanationBaseBord);
    this._scoreInfoBord = new Sprite(
      loadBitmapByTPokerPluginFolder("scoreGround")
    );
    ...
    this._cursolSprite = new Sprite(
      loadBitmapByTPokerPluginFolder("cursolFrame")
    );
    ...
    this._cursolStartPosSprite = new Sprite(
      loadBitmapByTPokerPluginFolder("startFrame")
    );
  ...
  Scene_PokerScene.prototype.createBackground = function () {
    this._backGroundImg = new Sprite(
      loadBitmapByTPokerPluginFolder(TatsuPokerGameP.backgroundImage)
    );
  ...
  Scene_PokerScene.prototype.isReady = function () {
    ...
    if (this._scoreInfoBord) {
      let isReadedScoreInfoBord =
        this._scoreInfoBord.width > 0 && this._scoreInfoBord.height > 0;
    ...
    if (this._royalStraightFlushAnimationSprite) {
      let isReadedRsfAns =
        this._royalStraightFlushAnimationSprite.width > 0 &&
        this._royalStraightFlushAnimationSprite.height > 0;
      if (!isReadedRsfAns) {
        return false;
      }
    }
  ...
  Scene_PokerScene.prototype.start = function () {
    ...
    this._scoreInfoBord.x = getCenterPos(
      Graphics.boxWidth,
      this._scoreInfoBord.width
    );
    this._scoreInfoBord.y = 10;
    this._scoreInfoWindow.x = this._scoreInfoBord.x;
    this._scoreInfoWindow.y = this._scoreInfoBord.y;
    this._royalStraightFlushAnimationSprite.x = getCenterPos(
      Graphics.boxWidth,
      640
    );
    this._royalStraightFlushAnimationSprite.y =
      getCenterPos(Graphics.boxHeight, 240) + 50;
    if (TatsuPokerGameP.useBGM) {
      $gameSystem.saveBgm();
      AudioManager.fadeOutBgm(1);
      if (TatsuPokerGameP.backBGM != "" && TatsuPokerGameP.backBGM != null) {
        AudioManager.playBgm({
          name: TatsuPokerGameP.backBGM,
          volume: 40,
          pitch: 100,
          pan: 0,
        });
  ...
  Scene_PokerScene.prototype.distributeCard = function () {
    ...
    if (
      TatsuPokerGameP.rsfPercentageSwitch &&
      ...
      if (rand <= $gameVariables.value(TatsuPokerGameP.rsfPercentage)) {
        ...
        if (
          TatsuPokerGameP.rsfCnt > 0 &&
          $gameVariables.value(TatsuPokerGameP.rsfCnt) > 0
        ) {
          this._royalStraightFlushCnt++;
          if (
            $gameVariables.value(TatsuPokerGameP.rsfCnt) <=
            this._royalStraightFlushCnt
          ) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (isRoyalStraightFlush) {
        ...
        while (
          indexList.indexOf(index) >= 0 ||
          rsfAddedMark.indexOf(this._allCardList[index].getMarkNum()) == -1
        ) {
          index = Math.floor(Math.random() * this._allCardList.length);
        }
      } else {
      ...
      cardInfo
        .getSprite()
        .setFrame(0, CARD_STATUS_OPEN_Y, CARD_IMG_WIDTH, CARD_IMG_HEIGHT);
  ...
  Scene_PokerScene.prototype.selectCardUpdateCommand = function () {
    if (!this._cardAnimationFlag) {
      ...
      if (this._animationTargetCardInfo.getIsOpen()) {
        ...
        this._animationTargetCardInfo
          .getSprite()
          .setFrame(0, framesetY, CARD_IMG_WIDTH, CARD_IMG_HEIGHT);
        if (framesetY <= CARD_STATUS_CLOSE_Y) {
        ...
        this._animationTargetCardInfo
          .getSprite()
          .setFrame(0, cntanimationTmp, CARD_IMG_WIDTH, CARD_IMG_HEIGHT);
        if (framesetY >= CARD_STATUS_OPEN_Y) {
          this._cardAnimationFlag = false;
  ...
  Scene_PokerScene.prototype.touchCard = function (touchX, touchY) {
    for (let i = 0; i < this._handCardList.length; i++) {
      ...
      if (
        touchX >= cardX &&
        touchX <= cardX + cardWidth &&
        touchY >= cardY &&
        touchY <= cardY + cardYHeight
      ) {
  ...
  Scene_PokerScene.prototype.touchStart = function (touchX, touchY) {
    ...
    if (
      touchX >= spriteX &&
      touchX <= spriteX + spriteWidth &&
      touchY >= spriteY &&
      touchY <= spriteY + spriteHeight
    ) {
      this._isSelectedCord = false;
      this.changeSelectedCard(this._isSelectedCord);
      this.cardChange();
    ...
    if (changeTargetCardNumberList.length > 0) {
      for (let i = 0; i < this._handCardList.length; i++) {
        if (changeTargetCardNumberList.indexOf(i) >= 0) {
          ...
          while (
            oldCardMarkNumberList.indexOf(
              this._allCardList[index].getMarkNum()
            ) >= 0 ||
            newCardMarkNumberList.indexOf(
              this._allCardList[index].getMarkNum()
            ) >= 0
          ) {
            index = Math.floor(Math.random() * this._allCardList.length);
  ...
  Scene_PokerScene.prototype.execCardChange = function () {
    ...
    let changeCardMaxIndex =
      this._changeTargetCardNumberList[
        this._changeTargetCardNumberList.length - 1
      ];
  ...
  Scene_PokerScene.prototype.checkRole = function () {
    ...
    switch (this._checkRole) {
      ...
      case ROLE_ROYALSTRAIGHTFLASH:
        AudioManager.playSe({
          name: TatsuPokerGameP.royalstraightFlushSe,
          volume: 80,
          pitch: 100,
          pan: 0,
        });
  ...
  // ロイヤルストレートフラッシュ
  Scene_PokerScene.prototype.checkRoyalStraightFlush = function (
    cardList,
    inJoker
  ) {
  ...
  Scene_PokerScene.prototype.checkStraightFlush = function (cardList, inJoker) {
    ...
    if (!inJoker) {
      for (let i = 1; i < cardList.length; i++) {
        if (
          minCard.getNum() + i == cardList[i].getNum() &&
  ...
  Scene_PokerScene.prototype.checkStraightPattern = function (cardList) {
    ...
    try {
      for (let i = 0; i < STRAIGHT_PATTERN.length; i++) {
        ...
        if (
          straightPt[0] === cardList[0].getNum() &&
  ...
  //3カード、4カード、、5カードの共通処理
  Scene_PokerScene.prototype.getCardDoubleCnt = function (
    cardList,
    num,
    inJoker,
    isFiveCard
      ...
      if (this._roleCommitAnimationCnt > 120) {
        if (this._checkRole != ROLE_HIGHCARD) {
          ...
          $gameVariables.setValue(
            TatsuPokerGameP.coinVariable,
            coinVal + this._winCoin
          );
          this._nowCoinWindows.reDrawCoin(
            $gameVariables.value(TatsuPokerGameP.coinVariable)
          );
  ...
  Window_MessageInfo_Window.prototype.initialize = function (
    x,
    y,
    width,
    height
  ...
  Window_MessageInfo_Window.prototype.drawMessage = function (text) {
    ...
    const arMax = function (a, b) {
      return Math.max(a, b);
    };
    let maxSize = messageSizeList.reduce(arMax);
    let tmpWindowWidth = maxSize + 50;
    let tmpWindowHeight =
      messageSizeList.length == 1
        ? this._baseHeight
        : messageSizeList.length * (this._baseHeight - 5);

    let tmpOldX = this.x;
    let tmpOldY = this._baseY;

    let tmpNewY =
      messageSizeList.length == 1 ? tmpOldY : tmpOldY - (this._baseHeight - 60);
