...
(function () {
  ...
  Game_Party.prototype.isGlossaryItem = function (item) {
    ...
    return (
      item.itypeId === iTypeId &&
      getMetaValues(item, ["説明", "Description"]) !== undefined
    );
  };
  ...
  Game_Party.prototype.gainGlossaryFromText = function (text) {
    this.getAllGlossaryList().forEach(
      function (item) {
        if (
          !this.hasItem(item) &&
    ...
    for (var i = 0; i < paramCommandNamesMax; i++) {
      if (paramCommandNames[i]) {
        this._commandWindow.setHandler(
          "glossary" + String(i),
          this.commandGlossary.bind(this, i + 1)
        );
      }
    }
  };
  ...
  Window_GlossaryList.prototype.isCategoryMatch = function (item) {
    return (
      !paramUseCategory ||
      this._category === $gameParty.getGlossaryCategory(item)
    );
  };
  ...
  Window_Glossary.prototype.calcMaxPages = function (depth) {
    ...
    var exist =
      !!getMetaValues(item, ["ピクチャ", "Picture"], depth) ||
    ...
    switch (this.getPicturePosition(item)) {
      ...
      case "bottom":
        ...
        this.drawPicture(
          ...
          bitmap,
          text,
          this.contentsHeight() - this.calcItemPictureHeight(item, bitmap, text)
        );
        break;
      default:
        this.drawItemText(text, 0);
  ...
  Window_Glossary.prototype.calcItemPictureHeight = function (
    item,
    bitmap,
    text
  ) {
    return bitmap
      ? bitmap.height * this.getPictureScale(item, bitmap, text) + 4
      : 0;
  };
