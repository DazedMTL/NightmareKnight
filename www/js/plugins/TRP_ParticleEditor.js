...
(function () {
  ...
  var displayCount =
    PluginManager.parameters(basePluginName).displayCount === "true";
  ...
  (function () {
    ParticleEditor.GUIDE_TEXTS = {
      ...
      burst: [
        "【バースト(発生方法)】",
        "particlesPerWaveで規則的に発生させる",
        "ps:射出角度の差(particleSpacing)",
        "sa:開始角度。ps*sa=360とすると放射状に射出(startAngle)",
        "dt:射出時間の差(deltaTime)",
        "rt:0=開始角度(startRotation)無視",
        "└1=開始角度優先,2=開始角度を加算(rotationType)",
        "r:半径(radius)",
        "dr:半径の差。マイナスで1周ごとの差(deltaRadius)",
        "dx:xの差(deltaX)",
        "dy:yの差(deltaY)",
        "<※発生方法はいずれか１つのみ指定が有効>",
      ],
      ...
      sub: [
        "【サブエミッター設定】",
        "st:射出開始タイミング(0~1)",
        "et:射出終了タイミング(0~1。-1で無効)",
        "spd:慣性の影響度合い。0=影響なし、1=100%影響。",
        "iAngle:1=開始角度は慣性方向基準",
        "iScale:1=親パーティクルの拡大率乗数を継承",
      ],
    };
  ...
  ParticleEditor.prototype.initialize = function (particle, id, exData) {
    ...
    if (id.contains(Game_Particle.SUB_EMITTER_DEV_SUFFIX)) {
      ...
      targetData = subData = particle.particleData(
        0,
        subId,
        "this",
        args[1],
  ...
  ParticleEmitter.prototype._updateParam = function (
    ...
    if (key === "sub") {
      ...
      if (Game_Particle.GROUP_EDITOR) {
        Game_Particle.GROUP_EDITOR.changeSubEmitterConfig(
          emitter.timing,
          emitter.endTiming,
          emitter.speedRate,
          emitter.inheritAngle,
          emitter.inheritScale
        );
      }
    } else {
...
===================================*/
  ...
  ParticleEditor.prototype.menuButtonSprite = function (
    ...
    sprite.x =
      Graphics.width - margin - index * (width + margin) - width - xOffset;
  ...
  ParticleEditor.prototype.checkImageExists = function (data) {
    ...
    var splitAnimImages =
      PluginManager.parameters(listPluginName).animImages || [];
...
===================================*/
  ...
  ParticleEditor.prototype.updateParticleCount = function () {
    ...
    if (!handler) {
      handler = SceneManager._scene.particleCountHandlerWithId(
        this._mainId,
        this._subId
      );
      if (handler) {
...
===================================*/
  ...
  ParticleEditor.prototype.writeSaveData = function (
    file,
...
===================================*/
  ...
  ParticleEditor.prototype.writeHelpFile = function () {
    ...
    for (var i = 0; i < length; i = (i + 1) | 0) {
      ...
      if (!isHidden) {
        ...
        file +=
          " * " + ParticleEditor.helpFileListText(key, targetType, comment);
      }
    ...
    var animImages =
      PluginManager.parameters(listPluginName).animImages || null;
  ...
  ParticleEditor.prototype.helpPluginCommandTexts = function (
    database = $dataTrpParticles
  ) {
    var texts = "";
...
===================================*/
  ...
  PickerBase.prototype._refresh = function () {
    ...
    this._dispRows = Math.floor(
      (Graphics.height - 2 * margin) / (itemHeight + my)
    );
...
===================================*/
  ...
  PickerBase.prototype.showHeaderSprite = function (
    header,
    color = "rgb(0,0,200)"
  ) {
...
===================================*/
  ...
  PresetPicker.prototype.isCategoryValid = function (index) {
    return (
      index === 0 || (this._allData[index] && this._allData[index].length > 0)
    );
  };
...
===================================*/
  ...
  PresetPicker.prototype.selectPrevious = function () {
    var index = this._selectingIndexes.length
      ? this._selectingIndexes[0]
      : this.maxItems();
    index -= 1;
    if (index < 0) {
      SoundManager.playBuzzer();
