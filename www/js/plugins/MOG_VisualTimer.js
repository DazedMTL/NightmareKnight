...
Moghunter.timerGauge_Number = String(
  Moghunter.parameters["Number Visible"] || "true"
);
Moghunter.timerGauge_Gauge = String(
  Moghunter.parameters["Gauge Visible"] || "true"
);
...
Moghunter.timerGauge_secX = Number(
  Moghunter.parameters["Seconds X-Axis"] || 132
);
Moghunter.timerGauge_secY = Number(Moghunter.parameters["Seconds Y-Axis"] || 0);
Moghunter.timerGauge_minX = Number(
  Moghunter.parameters["Minutes X-Axis"] || 82
);
Moghunter.timerGauge_minY = Number(Moghunter.parameters["Minutes Y-Axis"] || 0);
Moghunter.timerGauge_gaugeX = Number(
  Moghunter.parameters["Gauge X-Axis"] || 11
);
Moghunter.timerGauge_gaugeY = Number(
  Moghunter.parameters["Gauge Y-Axis"] || 34
);
Moghunter.timerGauge_AutoFade = String(
  Moghunter.parameters["Auto Fade"] || "true"
);
Moghunter.timerGauge_FadeMax = Number(Moghunter.parameters["Fade Max"] || 90);
Moghunter.timerGauge_GaugeAngle = Number(
  Moghunter.parameters["Gauge Angle"] || 0
);
Moghunter.timerGauge_NumberAngle = Number(
  Moghunter.parameters["Number Angle"] || 0
);
...
Game_System.prototype.initialize = function () {
  ...
  this._timerGauge.numberVisible =
    String(Moghunter.timerGauge_Number) === "true" ? true : false;
  this._timerGauge.gaugeVisible =
    String(Moghunter.timerGauge_Gauge) === "true" ? true : false;
  this._timerGauge.autoFade =
    String(Moghunter.timerGauge_AutoFade) === "true" ? true : false;
...
// ** create Spriteset
//==============================
var _mog_visualTimer_sbattle_createSpriteset =
...
Spriteset_Base.prototype.createTimer = function () {
  return;
  if ($gameSystem._timerGauge.enabled) {
    return;
  }
...
TimerGauge.prototype.initialize = function () {
  ...
  this.createSprites();
  this.update();
  if (this.data().autoFade && !this.isHudVisible()) {
    this.opacity = this.data().fadeMax;
  }
};
...
TimerGauge.prototype.loadBitmap = function () {
  ...
  this._numberImg = this.data().numberVisible
    ? ImageManager.loadSystem("Timer_Number")
    : new Bitmap(16, 16);
  this._gaugeImg = this.data().gaugeVisible
    ? ImageManager.loadSystem("Timer_Gauge")
    : new Bitmap(16, 16);
};
...
TimerGauge.prototype.createSprites = function () {
  this.createLayout();
  if (this.data().numberVisible) {
    this.createNumber();
  }
  if (this.data().gaugeVisible) {
    this.createGauge();
  }
};
...
TimerGauge.prototype.createNumber = function () {
  ...
  this._fieldNumber.rotation =
    (Moghunter.timerGauge_NumberAngle * Math.PI) / 180;
...
TimerGauge.prototype.seconds = function () {
  ...
  if (Math.floor($gameTimer._frames) > 0) {
    v += 1;
  }
  return v;
};
...
TimerGauge.prototype.updateGauge = function () {
  ...
  var meter_rate = Math.floor(
    (cw * $gameTimer._frames) / $gameTimer._framesMax
  );
...
TimerGauge.prototype.isHudVisible = function () {
  ...
  if ($gamePlayer.screen_realX() < this.x - this._dataImg1[0]) {
    return true;
  }
  if ($gamePlayer.screen_realX() > this.x + this._dataImg1[2]) {
    return true;
  }
  if ($gamePlayer.screen_realY() < this.y - this._dataImg1[1]) {
    return true;
  }
  if ($gamePlayer.screen_realY() > this.y + this._dataImg1[3]) {
    return true;
  }
...
TimerGauge.prototype.update = function () {
  ...
  if (this._dataImg1[0] != null) {
    ...
    if (this._numberImg.isReady() && this._layoutImg.isReady()) {
      this.getData();
    }
  }
};
