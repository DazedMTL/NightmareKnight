...
(function () {
  ...
  const pIfHpRecover = setDefault(
    parameters["ifHpRecover"],
    "b.hpRate() < 1.0"
  );
  const pIfMpRecover = setDefault(
    parameters["ifMpRecover"],
    "b.mpRate() < 1.0"
  );
  ...
  function isSpeedException(action) {
    ...
    // _cttbCountの有無で判定
    if (
      BattleManager._cttbCount ||
      BattleManager._isCttb ||
      BattleManager._isCtb
    ) {
      return false;
  ...
  Game_Enemy.prototype.isActionValid = function (action) {
    ...
    if (isTestApply(this)) {
      ...
      if (gameAction.isForUser()) {
        ...
        if (
          gameAction.isForOpponent() &&
          $gameParty.members().every(function (m) {
            return !gameAction.testApplyEnemy(m);
          })
        ) {
          return false;
          ...
          gameAction.isForFriend() &&
          $gameTroop.members().every(function (m) {
            return !gameAction.testApplyEnemy(m);
          })
        ) {
          return false;
