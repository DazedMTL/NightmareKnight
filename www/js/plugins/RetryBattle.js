...
(function () {
  ...
  BattleManager.payRetryCost = function () {
    ...
    if (param.RetryCostVariable > 0) {
      ...
      $gameVariables.setValue(
        param.RetryCostVariable,
        prevValue - param.RetryCostValue
      );
    }
  ...
  Scene_Gameover.prototype.drawMessage = function () {
    ...
    if (param.CostItemVariable > 0 && param.RetryCostItem > 0) {
      $gameVariables.setValue(
        param.CostItemVariable,
        $gameParty.numItems($dataItems[param.RetryCostItem])
      );
    }
  ...
  Window_RetryCommand.prototype.canPayRetryCost = function () {
    ...
    if (
      param.RetryCostVariable > 0 &&
      $gameVariables.value(param.RetryCostVariable) < param.RetryCostValue
    ) {
      return false;
    }
    if (
      param.RetryCostItem > 0 &&
      !$gameParty.hasItem($dataItems[param.RetryCostItem])
    ) {
      return false;
    }
    return true;
