...
DynamicDatabaseManager.makeDynamicDatabase = function () {
  ...
  this._iterate(
    this._targetDynamicDatabase,
    function (dataKey, columnMap) {
...
DynamicDatabaseManager._setColumnMapperDynamic = function () {
  this._getMaxLength("effects").times(
    function (i) {
      this._columnMapperCoomon["effects_%1_code".format(i)] =
        "使用効果%1_タイプ".format(i + 1);
      this._columnMapperCoomon["effects_%1_dataId".format(i)] =
        "使用効果%1_データID".format(i + 1);
      this._columnMapperCoomon["effects_%1_value1".format(i)] =
        "使用効果%1_内容1".format(i + 1);
      this._columnMapperCoomon["effects_%1_value2".format(i)] =
        "使用効果%1_内容2".format(i + 1);
    }.bind(this)
  );
  this._getMaxLength("traits").times(
    function (i) {
      this._columnMapperCoomon["traits_%1_code".format(i)] =
        "特徴%1_タイプ".format(i + 1);
      this._columnMapperCoomon["traits_%1_dataId".format(i)] =
        "特徴%1_データID".format(i + 1);
      this._columnMapperCoomon["traits_%1_value".format(i)] =
        "特徴%1_内容".format(i + 1);
    }.bind(this)
  );
  this._getMaxLength("actions").times(
    function (i) {
      this._columnMapperEnemies["actions_%1_conditionParam1".format(i)] =
        "行動%1_条件1".format(i + 1);
      this._columnMapperEnemies["actions_%1_conditionParam2".format(i)] =
        "行動%1_条件2".format(i + 1);
      this._columnMapperEnemies["actions_%1_conditionType".format(i)] =
        "行動%1_条件タイプ".format(i + 1);
      this._columnMapperEnemies["actions_%1_rating".format(i)] =
        "行動%1_R".format(i + 1);
      this._columnMapperEnemies["actions_%1_skillId".format(i)] =
        "行動%1_スキル".format(i + 1);
    }.bind(this)
  );
  Number(3).times(
    function (i) {
      this._columnMapperEnemies["dropItems_%1_kind".format(i)] =
        "ドロップアイテム%1_ドロップアイテム".format(i + 1);
      this._columnMapperEnemies["dropItems_%1_dataId".format(i)] =
        "ドロップアイテム%1_ドロップアイテムID".format(i + 1);
      this._columnMapperEnemies["dropItems_%1_denominator".format(i)] =
        "ドロップアイテム%1_出現率".format(i + 1);
    }.bind(this)
  );
};
...
DynamicDatabaseManager._makeProperty = function (parent, keyPath, key, child) {
  ...
  switch (typeof child) {
    ...
    case "number":
      ...
      var propName =
        this._columnMapper[keyPath] || this._columnMapperCoomon[keyPath];
...
DynamicDatabaseManager._makePropertyFormula = function (
  parent,
  key,
  child,
  metaTag
  ...
  Object.defineProperty(parent, key, {
    get: function () {
      var prev = child,
        元の値 = child;
      var result = eval(
        DynamicDatabaseManager._convertEscapeCharacters(this.meta[metaTag])
      );
...
DynamicDatabaseManager._convertEscapeCharacters = function (text) {
  ...
  text = text.replace(/\x1bS\[(\d+)\]/gi, function () {
    return $gameSwitches
      ? $gameSwitches.value(parseInt(arguments[1], 10))
        ? "1"
        : "0"
      : "0";
  });
