...
// * Dizze Effect
//==============================
Game_System.prototype.dizzeEffect = function (
  power,
  duration,
  blendType,
  opacity,
...
Scene_Map.prototype.canCallMenuDizzy = function () {
  if ($gameSystem._dizzyEffect.duration <= 20) {
    return true;
  }
  if ($gameTemp._dizzyEffectLoadTime > 0) {
    return false;
  }
  return true;
};
...
// * Execute Damage
//==============================
var _mog_dizzyEffect_gAction_applyItemUserEffect =
  Game_Action.prototype.applyItemUserEffect;
Game_Action.prototype.applyItemUserEffect = function (target) {
  _mog_dizzyEffect_gAction_applyItemUserEffect.call(this, target);
  if (this.canUseDizzyAnimation(target)) {
    this.executeDizzyAnimation(target);
  }
};
...
Scene_Map.prototype.terminate = function () {
  if (this._spriteset && this.dizzySprite() && this.dizzySprite()._sprites) {
    this.recordDizzyData();
  }
...
DizzySprites.prototype.prepareRefresh = function (sprite, i) {
  ...
  if (this.data().realTime) {
    this.refreshBitmap(sprite);
  }
  this._field.children.sort(function (a, b) {
...
DizzySprites.prototype.updateSprites = function (sprite, i) {
  ...
  if (sprite.wait > 0) {
    sprite.wait--;
    if (this.data().realTime && sprite.wait <= 0) {
      this.refreshBitmap(sprite);
    }
    return;
  }
  ...
  if (sprite.opacity === 0 && this.data().duration > 0) {
    this.prepareRefresh(sprite, i);
  }
};
...
DizzySprites.prototype.update = function () {
  ...
  if ($gameTemp._dizzyEffectLoadTime > 0) {
    $gameTemp._dizzyEffectLoadTime--;
  }
  if (this.data().refresh) {
    this.refreshDizzySprites(false);
  }
  if (
    !this._sprites &&
    this.dataSprites().length > 0 &&
    $gameTemp._dizzyEffectLoadTime === 0
  ) {
    this.refreshDizzySprites(true);
  }
  if (this.data().duration > 0 && !this.data().loop) {
    this.data().duration--;
  }
  if (this._sprites) {
    var needClear = true;
    ...
    if (this.needRemoveSprites(needClear)) {
      this.removeSprites();
    }
  }
};
